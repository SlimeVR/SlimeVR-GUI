      ...
      <EmptyModal isOpen={isOpen} onRequestClose={() => setOpen(false)}>
        <>
          <div className="flex w-full justify-center gap-3">
            ...
            <Button
              variant="primary"
              onClick={() => startProcess(AutoBoneProcessType.SAVE)}
              disabled={isProcessRunning || !hasRecording}
            >
              Save Recording
            </Button>
            <Button
            ...
            <div className="w-full rounded-full h-full overflow-hidden relative bg-purple-gray-800">
              <div
                className={classNames(
                  'h-full top-0 left-0 bg-purple-gray-300',
          ...
          <div className="flex flex-col w-full p-2">
            {bodyParts.map(({ label, bone, value }) => (
              <div
                key={bone}
                className="px-3 rounded-lg py-2 hover:bg-purple-gray-600"
              >
                <div className="flex flex-row gap-5">
                  <div className="flex flex-grow justify-start items-center text-field-title">
                    {label}
                  </div>
                  <div className="flex justify-center items-center w-16 text-field-title">{`${Number(
                    value * 100
                  )
                    .toFixed(1)
                    .replace(/[.,]0$/, '')}cm`}</div>
          ...
          <div className="flex w-full justify-between mt-3">
            ...
            <Button
              variant="primary"
              onClick={() => startProcess(AutoBoneProcessType.APPLY)}
              disabled={isProcessRunning || !hasCalibration}
            >
              Apply values
            </Button>
          </div>
